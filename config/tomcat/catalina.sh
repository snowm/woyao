...
get_var()
{
    sed -n -e "s/^$1=//p" server.properties | tr -d '\n\r'
}

get_var_with_default()
{
    RESULT=$(grep "^$1=" server.properties)
    if [ "$RESULT" ]; then
        verbose "$1 $RESULT"
...
assembl_java_opts()
{
  JVM_Xms=$(get_var jvm.Xms)
  JVM_Xmx=$(get_var jvm.Xmx)
  JVM_NewSize=$(get_var jvm.NewSize)
  JVM_MaxNewSize=$(get_var jvm.MaxNewSize)
  LOG_LEVEL=$(get_var log.level)
  LOG_DIR=$(get_var log.dir)
  FILE_ENCODING=$(get_var encoding)
  SPRING_PROFILE=$(get_var spring.profile)

  echo "----------------------------------------------- OPTS -----------------------------------------------"
  echo "Xms=$JVM_Xms"
  echo "Xmx=$JVM_Xmx"
  echo "NewSize=$JVM_NewSize"
  echo "MaxNewSize=$JVM_MaxNewSize"
